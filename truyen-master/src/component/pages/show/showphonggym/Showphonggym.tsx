...
const DataPhongym = importAll(
  require.context("component/img/imgphongym", true, /\.(png|jpe?g|svg)$/)
);
...
export default class ShowPhonggym extends Component {
  render() {
    const settings = {
      dots: false,
      infinite: true,
    ...
    return (
      <LinkCafe>
        <Link to="/detailShowgym" className="Linkdecafe">
          <h2 className="showmargin">Tranh vẽ tường phòng gym </h2>
          <TestHomslider>
            <Slider {...settings}>
              {DataPhongym.map((item: any) => {
                return (
