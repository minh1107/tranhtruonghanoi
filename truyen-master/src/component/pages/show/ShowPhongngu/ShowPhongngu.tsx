...
const DataPhongngu = importAll(
  require.context("component/img/imgtuongngu", true, /\.(png|jpe?g|svg)$/)
);
...
export default class ShowPhongNgu extends Component {
  render() {
    const settings = {
      dots: false,
      infinite: true,
    ...
    return (
      <LinkCafe>
        <Link to="/detailShowphongngu" className="Linkdecafe">
          <h2 className="showmargin">Tranh vẽ tường phòng ngủ cho bé </h2>
          <TestHomslider>
            <Slider {...settings}>
              {DataPhongngu.map((item: any) => {
                return (
